...
impl futhark_opaque_arr_t_1d {
   unsafe fn new<C>(ctx: C, arr: &[opaque_arr_t], dim: &[i64]) -> *const Self
   where C: Into<*mut bindings::futhark_context>
   {
     let ctx = ctx.into();
     bindings::futhark_new_opaque_arr_t_1d(
       ctx,
       arr.as_ptr() as *mut opaque_arr_t,
       dim[0],
)
     }
}

impl FutharkType for futhark_opaque_arr_t_1d {
   type RustType = opaque_arr_t;
   const DIM: usize = 1;

    unsafe fn shape<C>(ctx: C, ptr: *const bindings::futhark_opaque_arr_t_1d) -> *mut i64
    where C: Into<*mut bindings::futhark_context>
    {
        let ctx = ctx.into();
        bindings::futhark_shape_opaque_arr_t_1d(ctx, ptr as *mut bindings::futhark_opaque_arr_t_1d)
    }
    unsafe fn values<C>(ctx: C, ptr: *mut Self, dst: *mut Self::RustType)
    where C: Into<*mut bindings::futhark_context>
    {
        let ctx = ctx.into();
        bindings::futhark_values_opaque_arr_t_1d(ctx, ptr, dst);
    }
    unsafe fn free<C>(ctx: C, ptr: *mut Self)
    where C: Into<*mut bindings::futhark_context>
    {
        let ctx = ctx.into();
        bindings::futhark_free_opaque_arr_t_1d(ctx, ptr);
    }}
#[derive(Debug)]
...
#[derive(Debug)]
pub struct Array_opaque_arr_t_1d {
    ptr: *const futhark_opaque_arr_t_1d,
    ctx: *mut bindings::futhark_context,
}


impl Array_opaque_arr_t_1d {
    pub(crate) unsafe fn as_raw(&self) -> *const futhark_opaque_arr_t_1d {
         self.ptr
    }

    pub(crate) unsafe fn as_raw_mut(&self) -> *mut futhark_opaque_arr_t_1d {
         self.ptr as *mut futhark_opaque_arr_t_1d
    }
    pub(crate) unsafe fn from_ptr<T>(ctx: T, ptr: *const futhark_opaque_arr_t_1d) -> Self
        where
        T: Into<*mut bindings::futhark_context>,
    {
        let ctx = ctx.into();
    ...
    pub(crate) unsafe fn shape<T>(ctx: T, ptr: *const futhark_opaque_arr_t_1d) -> Vec<i64>
    where
        T: Into<*mut bindings::futhark_context>,
    {
        let ctx = ctx.into();
        let shape_ptr: *mut i64 = futhark_opaque_arr_t_1d::shape(ctx, ptr);
        let shape = std::slice::from_raw_parts(shape_ptr, 1);
        Vec::from(shape)
    }

    pub fn from_vec<T>(ctx: T, arr: &[opaque_arr_t], dim: &[i64]) -> Result<Self>
    where
        T: Into<*mut bindings::futhark_context>,
    {
        let expected = (dim.iter().fold(1, |acc, e| acc * e)) as usize;
        ...
        let ctx = ctx.into();
        unsafe {
            let ptr = futhark_opaque_arr_t_1d::new(ctx, arr, dim);
            Ok(Array_opaque_arr_t_1d { ptr, ctx })
        }
    }
    
    pub fn to_vec(&self) -> (Vec<opaque_arr_t>, Vec<i64>)
    {
        let ctx = self.ctx;
        unsafe {
            futhark_context_sync(ctx);
            let shape = Self::shape(ctx, self.as_raw());
            let elems = shape.iter().fold(1, |acc, e| acc * e) as usize;
            let mut buffer: Vec<opaque_arr_t> =
                vec![opaque_arr_t::default(); elems];
            let cint = futhark_opaque_arr_t_1d::values(ctx, self.as_raw_mut(), buffer.as_mut_ptr());
            (buffer, shape.to_owned())
        }
    }
    ...
    pub(crate) unsafe fn free_array(&mut self)
    {
        futhark_opaque_arr_t_1d::free(self.ctx, self.as_raw_mut());
    }
}

impl Drop for Array_opaque_arr_t_1d {
    fn drop(&mut self) {
        unsafe {
            self.free_array();
        }
